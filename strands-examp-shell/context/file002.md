...
source: "https://strandsagents.com/docs/user-guide/shell/configuration/"
author:
  - "[[Strands Agents]]"
published:
created: 2026-07-10
...
Every option is available both as a constructor argument and as a TOML key. Use the constructor when the policy is dynamic (per session, computed at runtime); use TOML when the policy is static and you want it under version control or shared with the [MCP server](https://strandsagents.com/docs/user-guide/shell/mcp-server/).
...
These caps are best-effort. They stop a runaway agent from exhausting memory or hanging; they aren’t a defense against an adversary actively trying to break out. For hard guarantees, see the [security model](https://strandsagents.com/docs/user-guide/shell/security/).

## TOML configuration

When the policy is static, declare it in a TOML file. The Python and Node.js constructors accept a config file path, and the [MCP server](https://strandsagents.com/docs/user-guide/shell/mcp-server/) reads the same format through its `--config` flag. Pass a file and explicit constructor arguments together, and the explicit arguments win.
...
- [Commands](https://strandsagents.com/docs/user-guide/shell/commands/): the full command inventory and supported flags.
- [MCP Server](https://strandsagents.com/docs/user-guide/shell/mcp-server/): serve this configuration to any MCP-compatible agent.
- [Security Model](https://strandsagents.com/docs/user-guide/shell/security/): how binds, credential injection, and the SSRF guard hold up, and where they stop.