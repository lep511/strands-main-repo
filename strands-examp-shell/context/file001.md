...
source: "https://strandsagents.com/docs/user-guide/shell/quickstart/"
author:
  - "[[Strands Agents]]"
published:
created: 2026-07-10
...
This starts a bare in-memory sandbox without access to files, network, or credentials. To grant access, write a [TOML config file](https://strandsagents.com/docs/user-guide/shell/configuration/#toml-configuration) and pass it before the `--mcp` flag:
...
The [MCP Server](https://strandsagents.com/docs/user-guide/shell/mcp-server/) page documents the four tools, their parameters, and how to expose other MCP servers as Lua modules inside the shell.
...
- [Configuration](https://strandsagents.com/docs/user-guide/shell/configuration/): the difference between `copy` and `direct` binds, credential injection, the network allowlist, and the TOML format.
- [Commands](https://strandsagents.com/docs/user-guide/shell/commands/): which commands and flags are supported, and where they diverge from GNU coreutils.
- [Security Model](https://strandsagents.com/docs/user-guide/shell/security/): what the sandbox guarantees, what it doesn’t, and when to add OS-level isolation.